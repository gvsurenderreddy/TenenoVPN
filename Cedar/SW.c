...
void SwInstallSettings(SW *sw, WIZARD_PAGE *wp)
{
	struct REMOTE_CLIENT *client;
	struct RPC_CLIENT_CREATE_VLAN t;
	struct RPC_CLIENT_CREATE_ACCOUNT a;
	...
	SwPerformPrint(wp, _UU("SW_PERFORM_MSG_SETTINGS"));
	
	//Defining default network adapter
	client = CcConnectRpc("localhost", "", 0, 0, 0);
	StrCpy(t.DeviceName, sizeof(t.DeviceName), "VPN10");
	CcCreateVLan(client, &t);

	//Defining default VPN connection
	Zero(&a, sizeof(a));
	a.ClientOption = ZeroMalloc(sizeof(CLIENT_OPTION));
	a.ClientAuth = ZeroMalloc(sizeof(CLIENT_AUTH));

	UniStrCpy(a.ClientOption->AccountName, sizeof(a.ClientOption->AccountName), CopyStrToUni("default"));
	StrCpy(a.ClientOption->Hostname, sizeof(a.ClientOption->Hostname),"default");
	a.ClientOption->Port = 443;
	StrCpy(a.ClientOption->HubName, sizeof(a.ClientOption->HubName), "default");
	StrCpy(a.ClientAuth->Username, sizeof(a.ClientAuth->Username),"username");
	a.ClientAuth->AuthType = CLIENT_AUTHTYPE_ANONYMOUS;

	CcCreateAccount(client, &a);
}
...
UINT SwWelcomeDlg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam, WIZARD *wizard, WIZARD_PAGE *wizard_page, void *param)
{
	...
	case WM_WIZ_SHOW:
		...
		sw->DoubleClickBlocker = false;
		sw->CurrentComponent = SwFindComponent(sw, SW_NAME_VPNCLIENT);
		break;
